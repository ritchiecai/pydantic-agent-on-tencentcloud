...
**部署形态**：单台 CVM 跑 FastAPI 应用，前置一个 CLB 做 HTTPS 终止与公网入口。所有云资源由 Terraform 管理。
...
| 网络 | 自建 **VPC + 子网 + 安全组** | 不用默认 VPC，符合 landing-zone 最佳实践；安全组仅放行 CLB→CVM 的 8000 端口 |
...
agent = Agent(
    MODEL_STRING,
    system_prompt="你是一个简洁的中文助手。需要时调用工具获取服务器本地时间。",
)
...
> 说明：只放 1 个 `server_time` 工具，目的是**证明 tool 机制与部署环境打通**（调用方能验证"agent 真的跑在腾讯云那台机器上"）。不放业务工具。
...
| 安全组 | `modules/vpc-security-group` | 仅放行 CLB→CVM:8000、出向 HTTPS（调模型 API） |
...
```hcl
output "service_url" { value = "https://${tencentcloud_clb_instance.main.clb_vip}" }  # 对外服务地址
...
6. **成本**：CVM + CLB 持续计费。**缓解**：用最小规格实例；README 提供 `terraform destroy` 一键回收说明。
